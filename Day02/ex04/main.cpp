...
Fixed calculate(std::string expr) {
	...
	else if (expr.find('-', 1) != std::string::npos && (pos =  expr.find('-', 1))
	...
	{
		pos = expr.find('/');
		Fixed div1 = calculate(expr.substr(0, pos));
		Fixed div2 = calculate(expr.substr(pos + 1));
		if (div2 == 0)
		{
			exit (EXIT_FAILURE);
		}
...
bool valideExpr(std::string expr)
{
	...
	for (uint i = 0; i < expr.length(); ++i)
	{
		...
		else if (expr[i + 1] && tockens.find(expr[i], 3) != std::string::npos &&
				tockens.find(expr[i + 1], 5) != std::string::npos)
		{
			std::cout << "BAD TOCKEN ["<< expr[i] << expr[i + 1] << "]" << std::endl;
			return false;
		}
		if (expr[i] == '(')
			n++;
